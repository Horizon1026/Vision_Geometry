...
bool IcpSolver::EstimatePoseByMethodPointToLine(const std::vector<Vec3> &all_ref_p_w,
    ...
    for (uint32_t iter = 0; iter < options_.kMaxIteration; ++iter) {
        ...
        for (const auto &cur_p_w : all_cur_p_w) {
            ...
            CONTINUE_IF(it->first > options_.kMaxValidRelativePointDistance && std::next(it)->second > options_.kMaxValidRelativePointDistance);
        ...
        // Check if converged.
        BREAK_IF(dq_rc.vec().norm() + dp_rc.norm() < 1e-6f);
    }
