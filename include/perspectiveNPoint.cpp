...
void PerspectiveNPointClass::EstimateRotationAndTranslation(std::vector<cv::Point3f> &wordPoints,   // 对应世界坐标系中的 3D 点坐标
    ...
    std::vector<Eigen::Vector3d> points2D = GetNormPointsFromPixelPoints(pixelPoints, CameraMatrix);

    // 调用接口函数
    PerspectiveNPointClass::EstimateRotationAndTranslation(points3D, points2D, R_cw, t_cw, status);
}
...
void PerspectiveNPointClass::EstimateRotationAndTranslation(std::vector<Eigen::Vector3d> &wordPoints,  // 对应世界坐标系中的 3D 点坐标
    ...
    while (maxEpoch > 0) {
        ...
        if (PerspectiveNPointClass::IsCollineation(tempPoints2D) == true) {
            continue;
        }
        ...
        PerspectiveNPointClass::SolvePnP(tempPoints3D, tempPoints2D, tempR, tempt);
    
        // 根据当前的模型 R 和 t，计算出所有点对的模型误差
        tempErrors = ComputeReprojectionError(wordPoints, normPoints, tempR, tempt);
        ...
        if (inliersMeanError < minError) {
            ...
            if (inliersPoints3D.size() > 3) {
                PerspectiveNPointClass::SolvePnP(inliersPoints3D, inliersPoints2D, tempR, tempt);
                bestR = tempR;
                bestt = tempt;
            } else {
                bestR = tempR;
...
void PerspectiveNPointClass::SolvePnP(std::vector<cv::Point3f> &wordPoints,     // 对应世界坐标系中的 3D 点坐标
    ...
    // 调用接口函数
    PerspectiveNPointClass::SolvePnP(points3D, points2D, R_cw, t_cw);
}
